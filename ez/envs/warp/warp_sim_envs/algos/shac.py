...
from . import shac_utils
from .shac_utils import RunningMeanStd, grad_norm, print_info, seeding
from .shac_utils import CriticDataset, TimeReport, AverageMeter
...
class SHAC:
    def __init__(
        ...
        adam_betas=(0.7, 0.95),
        lambda_=0.95,
        actor_network="ActorStochasticMLP",
        actor_network_params=dict(
            hidden_layers=[64, 64],
            activation="elu",
        ),
        actor_learning_rate=1e-2,
        critic_network="CriticMLP",
        critic_network_params=dict(
            hidden_layers=[64, 64],
            activation="elu",
        ),
        ...
        self.device = torch.device(wp.device_to_torch(self.env.device))

        self.gamma = gamma

        self.critic_method = critic_method  # ['one-step', 'td-lambda']
        if self.critic_method == "td-lambda":
            self.lam = lambda_

        self.max_epochs = max_epochs
        self.actor_lr = float(actor_learning_rate)
        self.critic_lr = float(critic_learning_rate)
        ...
        if train:
            ...
            self.stochastic_evaluation = not deterministic

        # create actor critic network
        self.actor_name = (
            actor_network  # choices: ['ActorDeterministicMLP', 'ActorStochasticMLP']
        )

        self.critic_name = critic_network  # choices: ['CriticMLP']
        actor_fn = getattr(shac_utils, self.actor_name)
        self.actor = actor_fn(
            self.num_obs,
            self.num_actions,
            device=self.device,
        ...
        critic_fn = getattr(shac_utils, self.critic_name)
        self.critic = critic_fn(
            self.num_obs, device=self.device, **critic_network_params
        )
        ...
        self.obs_buf = torch.zeros(
            (self.num_steps, self.num_envs, self.num_obs),
            dtype=torch.float32,
            device=self.device,
        )
        self.rew_buf = torch.zeros(
            (self.num_steps, self.num_envs), dtype=torch.float32, device=self.device
        )
        ...
        self.old_mus = torch.zeros(
            (self.num_steps, self.num_envs, self.num_actions),
            dtype=torch.float32,
            device=self.device,
        )
    ...
    def compute_actor_loss(self, deterministic=False):
        rew_acc = torch.zeros(
            (self.num_steps + 1, self.num_envs), dtype=torch.float32, device=self.device
        )
        ...
        actor_loss /= self.num_steps * self.num_envs
        ...
        self.step_count += self.num_steps * self.num_envs
    ...
    def play(self, checkpoint: str, num_games: int = 10):
