...
def plot_distribution(data_2d: np.ndarray, show_min_max: bool = True):
    assert data_2d.ndim == 2
    # make violin plots of the joint angles and velocities
    import matplotlib.pyplot as plt
    import seaborn as sns

    # show min / max whiskers
    fig, ax = plt.subplots(1, 1, figsize=(15, 5))
    sns.violinplot(data=data_2d, ax=ax)
    q_min = np.min(data_2d, axis=0)
    q_max = np.max(data_2d, axis=0)
    if show_min_max:
        # show min and max
        for i in range(data_2d.shape[1]):
            ax.plot([i - 0.5, i + 0.5], [q_min[i], q_min[i]], color="black")
            ax.plot([i - 0.5, i + 0.5], [q_max[i], q_max[i]], color="black")
